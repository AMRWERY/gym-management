<template>
  ...
  <!-- Content Container -->
  <div
    class="min-h-screen bg-color-light-gray"
    :class="{ 'overflow-hidden max-h-screen fixed': visibleResponsiveSidebar }">
    <navbar />
    <!-- Content -->
    <div class="lg:ms-64 pt-14 px-5">
      <div v-if="display_loader" wire:loading class="bg-black fixed top-0 bottom-0 -mx-5 w-full h-screen z-50 opacity-50 flex flex-col items-center justify-center">
        <svg class="lg:-ms-64 animate-spin h-12 w-12 rounded-full bg-transparent border-2 border-transparent border-opacity-50 loader" viewBox="0 0 24 24"></svg>
      </div>
