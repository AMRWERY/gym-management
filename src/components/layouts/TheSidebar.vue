<template>
  ...
  <button 
    class="fixed z-50 bottom-4 end-4 w-16 h-16 rounded-full bg-gray-900 lg:hidden"
    @click="toggleSidebar">
    <tw-icon 
  ...
  <div 
    class="fixed z-40 start-0 top-0 end-24 lg:start-auto lg:w-64 h-screen border-r border-gray-200 overflow-y-auto bg-white" 
    :class="{'hidden': allowSidebarHide && !showSidebar}">
    <!-- Header -->
    <header class="flex items-center py-3 px-4 bg-gray-100">
      <img 
        src="@/assets/logo_sidebar.png" 
        alt="Logo" 
        class="h-8">
        <span class="text-md font-medium text-gray-600 pt-1 ps-2">{{ $t('app.name') }}</span>
    </header>
    <!-- Menu -->
    <div class="mt-2">
      ...
      <menu-item
        ...
        :text="lang == 'ar' ? admin_menu.title_ar : admin_menu.title_en" 
        :route="{path: admin_menu.link}">
          <template v-if=" admin_menu.sub_menus?.length > 0" #sub-menu>
            <sub-menu-item 
              ...
              :key="sub_menu.id"
              :text="lang == 'ar' ? sub_menu.title_ar : sub_menu.title_en" 
