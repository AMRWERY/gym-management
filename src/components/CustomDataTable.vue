		...
		<div>
			...
			<div v-if="enableHeader" class="mb-2">
				<div class="grid grid-cols-2 gap-4">
					<div>
						<div v-show="searchBox" class="shadow flex md:me-44" :class="searchBoxClass">
							<input v-model="search_keyword" v-on:input="onSearch" class="w-full rounded p-2" type="text"
					...
					<div v-show="isAddNew || isImportData" class="mx-2"
						:class="language == 'en' ? 'text-right' : 'text-left'">
						<button v-show="isImportData" @click="onImportBtnClicked()"
							class="ms-2 btn-primary rounded px-2 py-1.5 text-md inline">
							{{ importBtnText }}
						</button>

						<button v-show="isAddNew" @click="onAddBtnClicked()"
							class="ms-2 btn-primary rounded px-2 py-1.5 text-md">
							{{ addBtnText }}
						</button>
					</div>
				</div>
			...
			<!-- Table -->
			<div class="table-auto border-b border-gray-200 shadow bg-white">
				<table id="custom-data-table" class="divide-y divide-gray-300 w-full" :class="tableExtStyle">
					<thead>
						<tr class="text-start">
							<th class="px-2 py-3 z-1" :class="column.colHeadStyle ? column.colHeadStyle : ''"
								v-for="column in columns" :key="`columns-${column.name}`">
								<a class="text-xs text-gray-500 flex" href="#" v-if="column.sortable"
									@click.prevent="sort(this.sorted_order, column.attribute, column.dataType)">
								...
								<span :id="column.name == 'Action' ? 'datatable-th-action' : ''" v-else
					...
					<tbody class="divide-y divide-gray-300">
						<tr :class="highlightRow && !resource.is_viewed ? 'bg-highlight-row' : isOrders && resource.order_status == 'rejected' ? 'text-red-500' : ''"
							v-for="(resource, index) in paginatedData" :key="resource.id" class="text-gray-500">
							<td class="px-2 py-3 text-sm" v-for="column in columns"
								...
								<slot class="text-sm" name="cell" v-bind="{ column, resource }">
									...
									<span v-else-if="column.attribute == 'is_active' && column.displayText == true">
										<div>
											<span class="text-sm" v-if="resource[column.attribute]">{{
												$t('datatable.active')
											}}</span>
											<span class="text-sm" v-else>{{ $t('datatable.inactive') }}</span>
										</div>
									</span>
									<span v-else-if="column.type == 'custom_toggle'">
										<label class="switch cursor-pointer">
											<input type="checkbox"
												@click='column.clickEvent($event, resource.id, resource)'
												v-model="resource[column.attribute]">
									...
									<span v-else-if="column.attribute == 'is_active' || column.display == 'icon'">
										<div v-if="quickAction">
											<label class="switch cursor-pointer">
												<input type="checkbox"
													@click='onQuickActionPerformed($event, resource.id, resource)'
													v-model="resource[column.attribute]">
												<span class="slider round"></span>
											</label>
										</div>
										<div v-else>
											<span
												class="text-xs rounded-md text-white text-bg-green px-3 py-1 uppercase"
												v-if="resource[column.attribute]">{{ $t('datatable.active') }}</span>
											<span class="text-xs rounded-md text-white bg-red-600 px-3 py-1 uppercase"
												v-else>{{ $t('datatable.inactive') }}</span>
										</div>
									</span>
									<span v-else-if="column.attribute == 'is_valid'">
										<div>
											<span
												class="text-xs rounded-md text-white text-bg-green px-3 py-1 uppercase"
												v-if="resource[column.attribute]">{{ $t('datatable.valid') }}</span>
											<span class="text-xs rounded-md text-white bg-red-600 px-3 py-1 uppercase"
												v-else>{{ $t('datatable.invalid') }}</span>
										</div>
									</span>
									<span id="data-table-action-buttons" v-else-if="column.attribute == 'action'"
										class="flex" :class="hasActionButtonLayoutStyle ? actionButtonLayoutStyle : ''">

										<a v-show="view" class="inline-block cursor-pointer"
											@click='onView(resource.id, resource)'>
											<tw-icon name="heroicons-outline:eye" class="w-6 h-6 color-primary" />
										</a>
										<a v-show="edit" class="inline-block ps-2 cursor-pointer"
											@click='onEdit(resource.id, resource)'>
											<tw-icon name="heroicons-outline:pencil-alt"
												class="w-6 h-6 text-blue-400" />
										</a>
										<a v-show="del" class="inline-block ps-2 cursor-pointer" :class="resource.style"
											@click='onDeleteIconClicked(resource)'>
											<tw-icon name="heroicons-outline:trash" class="w-6 h-6"
												:class="resource.del_icon_style ?? 'text-red-400'" />
										</a>
										<div v-show="hasCustomAction">
											<span @click="onCustomActionClicked(resource.id, resource)"
												class="p-2 ms-2 block text-sm bg-primary text-white rounded cursor-pointer"
												:class="resource.style">{{ customActionBtnText }}</span>
										</div>
										<tw-dropdown v-show="dropdownAction" class="ms-2" origin="right"
											body-class="top-8 border-none">
											<div
												class="flex items-center cursor-pointer text-sm font-medium text-center text-gray-900 bg-white rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none dark:text-white focus:ring-gray-50 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-600">
											...
											<template #body>
												<ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
													<li v-for="(dropdownMenu, index) in dropdownMenus" :key="index"
														@click="dropdownMenu.function(resource)"
														class="px-4 py-2 cursor-pointer border profile-links">
														{{ dropdownMenu.name }}
													</li>
												</ul>
											</template>
										...
										<a v-show="!hasCustomAction && !dropdownAction && !view && !edit && !del"
											class="inline-block ps-2">
											—
										</a>
									</span>
									<span v-else-if="column.field == 'image'">
										<img class="w-8 rounded" :src="resource[column.attribute]" />
									</span>
									<span v-else-if="column.nestedObject">
										<span v-if="column.removeUnderscore"><span v-if="column.isPrice">{{
											column.prefix
										}} {{
											removeUnderscore(resource[column?.nestedObject][column?.attribute])?.toFixed(3)
										}}</span><span v-else>{{ column.prefix }} {{
											removeUnderscore(resource[column?.nestedObject][column?.attribute])
										}}</span></span> <span v-else><span v-if="column.isPrice">{{
											column.prefix
										}} {{
											resource[column?.nestedObject][column?.attribute]?.toFixed(3)
										}}</span><span v-else>{{ column.prefix }} {{
											resource[column?.nestedObject][column?.attribute]
										}}</span></span>
									</span>
									<span v-else-if="column.nestedArrayObject" class="">
										<span v-if="column.removeUnderscore">
											<span v-if="column.isPrice">{{ column.prefix }} {{
												removeUnderscore(resource[column?.nestedArrayObject][0][column?.attribute])?.toFixed(3)
											}}</span>
											<span v-else>
												<pre class="font-tajwal"
													v-html="removeUnderscore(getCommaSeparatedValuesFromArray(resource[column?.nestedArrayObject], [column?.attribute]))"></pre>
											</span>
										</span>
										<span v-else>
											<span v-if="column.isPrice">{{ column.prefix }} {{
												resource[column?.nestedArrayObject][0][column?.attribute]?.toFixed(3)
											}}</span>
											<span v-else>
												<pre class="font-tajwal"
													v-html="getCommaSeparatedValuesFromArray(resource[column?.nestedArrayObject], [column?.attribute])"></pre>
											</span>
										</span>
									</span>
									<span v-else :class="column.extStyle">
										<span :class="resource[column.resource_style]">
											<span v-if="column.removeUnderscore"><span v-if="column.isPrice">{{
												column.prefix
											}} {{
												resource[column.attribute]?.toFixed(3)
											}}</span><span v-else>{{ column.prefix }} {{
												removeUnderscore(resource[column.attribute])
											}}</span></span> <span
												v-else><span v-if="column.isPrice">{{ column.prefix }} {{
													resource[column.attribute]?.toFixed(3)
												}}</span><span v-else>{{
												column.prefix
											}} {{ resource[column.attribute] }}</span></span>
										</span>
									</span>
								</slot>
							</td>
			...
			<div id="dataTableFooter" class="mt-5 mx-2">
				<div v-if="!serverSidePagination" class="flex items-center pb-8">
					<span class="me-3 text-md">{{ $t('datatable.showing') }}</span>
					<form @submit.prevent>
						<select class="mt-1  py-1.5 pe-1 border rounded" v-model="page_size" @change="onChange($event)">
							<option v-for="perPageOption in per_page_options" :key="`per-page-options-${perPageOption}`"
								:value="perPageOption">
								{{ perPageOption }}
							</option>
						</select>
					</form>
					<!-- <VueTailwindPagination class="ms-auto flex h-12" :current="current" :total="rows_data.length"
						:per-page="page_size" @page-changed="current = $event" /> -->

					<vue-awesome-paginate
						class="ms-auto flex h-12"
						:total-items="rows_data.length"
				...
				<div v-else class="flex items-center pb-8">
					<span class="me-3 text-md">{{ $t('datatable.showing') }}</span>
					<form @submit.prevent>
						<select class="mt-1 py-1.5 pe-1.5 border rounded" v-model="default_page_size"
							@change="onPerPageOptionChange($event, search_keyword, data_sort_order, data_sort_by)">
							<option v-for="perPageOption in per_page_options" :key="`per-page-options-${perPageOption}`"
								:value="perPageOption">
								{{ perPageOption }}
							</option>
					...
					<VueTailwindPagination class="ms-auto flex h-12" :current="pagination_data?.current_page"
						:total="pagination_data?.total" :per-page="pagination_data?.per_page"
