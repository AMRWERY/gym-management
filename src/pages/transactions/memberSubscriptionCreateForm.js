...
export default {
  ...
  methods: {
    ...
    setSubscriptionStartDate() {
      if (this.last_subscription) {
        this.subscription.start_date = this.$root.addDays(
          this.$root.formatDateUI(this.last_subscription?.end_date),
          1
        );
        this.locker_detail.start_date = this.subscription.start_date;
        this.lesson_detail.start_date = this.subscription.start_date;
        this.service_detail.start_date = this.subscription.start_date;
      }
    },
    ...
    clearHealthDetail() {
      ...
      this.health_detail.as_on_date = this.subscription.start_date;
    ...
    setLockerPackageEndDate() {
      if (this.subscription.membership_type_id == 1) {
        this.subscription.end_date = this.$root.addDays(
    ...
    getMembershipPackages() {
      let membership_type_id = this.subscription.membership_type_id;
    ...
    getLockerPackages() {
      let locker_type_id = this.locker_detail.locker_type_id;
      this.locker_packages_list = [];
      this.lockers_list = [];
    ...
    getServicePackages() {
      let service_type_id = this.service_detail.service_type_id;
      this.service_packages_list = [];
    ...
    getLessonPackages() {
      let lesson_type_id = this.lesson_detail.lesson_type_id;
      this.lesson_packages_list = [];
    ...
    setLockerPackageAmount() {
      let locker_package_id = this.locker_detail.locker_package_id;
    ...
    setServicePackageAmount() {
      let service_package_id = this.service_detail.service_package_id;
    ...
    setMembershipPackageAmount() {
      let membership_package_id = this.subscription.membership_package_id;
      this.free_lockers = [];
      this.free_services = [];
      this.free_lessons = [];
      this.subscription.membership_package_amount = 0;
      this.subscription.total_membership_amount = 0;
      ...
      this.membership_packages.forEach((membership_package) => {
        if (membership_package_id == membership_package.id) {
          ...
          if (membership_package.locker_packages.length > 0) {
            membership_package.locker_packages.forEach((locker_package) => {
              this.free_lockers.push(locker_package);
            });
          }

          if (membership_package.service_packages.length > 0) {
            membership_package.service_packages.forEach((service_package) => {
              this.free_services.push(service_package);
            });
          }

          if (membership_package.lesson_packages.length > 0) {
            membership_package.lesson_packages.forEach((lesson_package) => {
              this.free_lessons.push(lesson_package);
            });
          }
    ...
    setFreeLocker() {
      if (this.free_lockers.length > 0) {
        this.free_lockers.forEach((free_locker) => {
          this.locker_detail.locker_type_id = free_locker?.locker_type_id;
          this.getLockerPackages();
          this.locker_detail.locker_package_id = free_locker?.id;
          this.setLockerPackageAmount();
        });
      }
    },
    ...
    setFreeService() {
      if (this.free_services.length > 0) {
        this.free_services.forEach((free_service) => {
          this.service_detail.service_type_id = free_service?.service_type_id;
          this.getServicePackages();
          this.service_detail.service_package_id = free_service?.id;
          this.setServicePackageAmount();
        });
      }
    },

    setFreeLesson() {
      if (this.free_lessons.length > 0) {
        this.free_lessons.forEach((free_lesson) => {
          this.lesson_detail.lesson_type_id = free_lesson?.lesson_type_id;
          this.getLessonPackages();
          this.lesson_detail.lesson_package_id = free_lesson?.id;
          this.setLessonPackageAmount();
        });
      }
    },

    setLessonPackageAmount() {
      let lesson_package_id = this.lesson_detail.lesson_package_id;
  ...
  watch: {
    subscriptions(data) {
      if (data != null) {
        ...
        this.last_subscription = data.subscriptions[0];
