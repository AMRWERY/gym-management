    ...
    <div>
        <div v-if="has_invoice_permission && subscription_details" class="bg-white" id="bookingInfo">
            ...
            <div class="grid grid-cols-1 py-4 px-8">
                <h5 class="font-bold text-md text-center mb-3">REFUND DETAIL REPORT</h5>
                ...
                <div class="grid grid-cols-2 justify-content-between">
                    ...
                    <p class="font-medium text-right">{{ subscription_details?.refund_invoice?.created_at ?? '-'}}</p>
                </div>
            </div>

            <div class="grid grid-cols-1 px-8">
                <div class="grid grid-cols-5 justify-content-between py-1 px-2 border-t-2">
                    <p class="text-sm font-bold col-span-2">Refund Type</p>
                    <p class="text-sm font-bold col-span-2">Package</p>
                    <p class="text-sm font-bold text-right">Refund Amount</p>
                </div>
            </div>

            <div class="grid grid-cols-1 px-8 pb-4">
                <div class="grid grid-cols-5 justify-content-between py-1 px-2">
                    <p class="text-sm font-medium col-span-2">{{ subscription_details?.refund_invoice?.type }}</p>
                    <p class="text-sm font-medium col-span-2">{{ subscription_details?.refund_invoice?.description_en }}</p>
                    <p class="text-sm font-medium text-right">{{ $t('app.currency') }}
                        {{ subscription_details?.refund_invoice?.refund_amount?.toFixed(3) ?? '0.000'}}</p>
                </div>
            </div>
...
export default {
    data() {
        return {
            v$: useValidate(),
            invoice_refund_id: this.$route.params.id,
    ...
    computed: {
        ...mapGetters(["refundInvoiceDetails"]),
    },
    methods: {
        ...mapActions(["getRefundInvoiceDetails"]),
        printBookingDetails() {
            const options = {
                name: "Platinum Gym",
                specs: [],
        ...
        checkPermissions() {
            ...
            this.has_invoice_permission = this.$root.getPermissionStatus(page_permissions?.permissions, "refund_invoice_details")
        },

    },
    watch: {
        refundInvoiceDetails(data) {
            if (data != null) {
    ...
    mounted() {
        ...
        this.getRefundInvoiceDetails(this.invoice_refund_id)
    },
}
</script>