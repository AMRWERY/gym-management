<template>
    <div>
        <div v-if="has_invoice_permission && subscription_details" class="bg-white" id="bookingInfo">
            ...
            <div class="grid grid-cols-1 py-4 px-8">
                ...
                <div class="grid grid-cols-2 justify-content-between">
                    ...
                    <p class="font-bold text-right color-primary">{{ subscription_details?.member_name_en ?? '-' }} / {{
                        subscription_details?.member_name_ar ?? '-' }}</p>
                </div>
                <div class="grid grid-cols-2 justify-content-between">
                    <p class="font-medium">{{ $t('transactions.member_id') }}</p>
                    <p class="font-medium text-right">{{ subscription_details?.member_code ?? '-' }}</p>
                </div>
                <div class="grid grid-cols-2 justify-content-between">
                ...
                <div class="grid grid-cols-2 justify-content-between">
                    ...
                    <p class="font-medium text-right">{{ subscription_details?.subscription_freeze?.created_at ?? '-' }}</p>
                </div>
            </div>


            <div class="grid grid-cols-1 px-8 pb-4">
                <h2 class="text-sm font-bold border-b-2 color-primary">{{ $t('transactions.freeze_details') }}</h2>

                <div class="grid grid-cols-5 justify-content-between py-1 px-2">
                    <p class="text-sm font-medium col-span-2">{{ subscription_details?.subscription_freeze?.remark }}</p>
                    <p class="text-sm font-medium">{{ $t('transactions.from') }} {{
                        subscription_details?.subscription_freeze?.start_date ??
                        '-' }}</p>
                    <p class="text-sm font-medium">{{ $t('transactions.to') }} {{
                        subscription_details?.subscription_freeze?.end_date ?? '-' }}</p>
                    <p class="text-sm font-medium text-right color-primary">{{
                        subscription_details?.subscription_freeze?.freeze_days ??
                        '-' }} {{ $t('transactions.days') }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 px-8 pb-4">
                ...
                <div class="grid grid-cols-5 justify-content-between py-1 px-2">
                    <p class="text-sm font-medium col-span-2">{{ subscription_details?.membership_package?.name_en }}</p>
                    <p class="text-sm font-medium">{{ $t('transactions.from') }} {{ subscription_details?.start_date ??
                        '-' }}</p>
                    <p class="text-sm font-medium">{{ $t('transactions.to') }} {{
                        subscription_details?.subscription_freeze?.new_subscription_end_date ??
                        '-' }}</p>
            ...
            <div class="grid grid-cols-1 px-8 pb-4" v-if="subscription_details?.subscription_service?.length">
                ...
                <div v-for="(service, index) in subscription_details.subscription_service" :key="index"
                    ...
                    <p class="text-sm font-medium">{{ $t('transactions.from') }} {{ service?.start_date ?? '-' }}</p>
                    <p class="text-sm font-medium">{{ $t('transactions.to') }} {{ service?.end_date ?? '-' }}</p>
            ...
            <div class="grid grid-cols-1 px-8 pb-4" v-if="subscription_details?.subscription_locker?.length">
                ...
                <div v-for="(locker, index) in subscription_details.subscription_locker" :key="index"
                    class="grid grid-cols-5 justify-content-between py-1 px-2">
                    <p class="text-sm font-medium col-span-2">{{ locker?.locker_package?.name_en }}<br><span>{{
                        $t('transactions.insurance_amount')
                    }}: {{ $t('app.currency') }}
                            {{ locker.insurance_amount?.toFixed(3) }}</span></p>
                    <p class="text-sm font-medium">{{ $t('transactions.from') }} {{ locker?.start_date ?? '-' }}</p>
                    <p class="text-sm font-medium">{{ $t('transactions.to') }} {{ locker?.end_date ?? '-' }}</p>
            ...
            <div class="grid grid-cols-1 px-8" v-if="subscription_details?.subscription_lesson?.length">
                ...
                <div v-for="(lesson, index) in subscription_details.subscription_lesson" :key="index"
                    ...
                    }}: {{ lesson?.lesson_package?.no_of_classes }}</span></p>
                    <p class="text-sm font-medium">{{ $t('transactions.from') }} {{ lesson?.start_date ?? '-' }}</p>
                    <p class="text-sm font-medium">{{ $t('transactions.to') }} {{ lesson?.end_date ?? '-' }}</p>
            ...
            <div class="grid grid-cols-1 py-4 px-8" v-if="subscription_details?.subscription_payment?.length">
                ...
                <div v-for="(payment, index) in subscription_details.subscription_payment" :key="index"
                    ...
                    <p class="text-sm font-medium text-right">{{ $t('app.currency') }} {{ payment?.amount ?? '0.000' }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 px-8 py-4 hidden print:grid">
                ...
                <div class="flex">
                    ...
                    <p class="text-sm font-medium"> {{ printed_by }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white py-4 px-3 border border-gray-200 mb-1 gap-2 grid grid-cols-2">
            ...
            <button type="button" @click="cancelInvoice" class="w-full justify-center border-1 border-color-primary 
        ...
        <MemberSubscriptionCancelFreezeForm v-if="show_subscription_cancel_freeze_from"
            :selected_subscription="subscription_details" @close-modal="show_subscription_cancel_freeze_from = false" />
    </div>
</template>
...
import MemberSubscriptionCancelFreezeForm from "./MemberSubscriptionCancelFreezeForm.vue";

export default {
    components: { MemberSubscriptionCancelFreezeForm },
    data() {
        return {
            v$: useValidate(),
            show_subscription_cancel_freeze_from: false,
            selected_subscription: [],
            invoice_freezed_id: this.$route.params.id,
            subscription_details: [],
            invoice_details: [],
            printing: false,
            branch: [],
            todays_date: new Date().toLocaleDateString('fr-CA'),
            cancel_remark: '',
            has_invoice_permission: false,
            printed_on: moment().format('DD-MM-YYYY hh:mm a'),
            printed_by: JSON.parse(localStorage.getItem("userInfo"))?.user.name
        }
    },
    computed: {
        ...mapGetters(["freezedInvoiceDetails"]),
    },
    methods: {
        ...mapActions(["getFreezedInvoiceDetails"]),

        printBookingDetails() {
            const options = {
                name: "Platinum Gym - Freezed Invoice",
                specs: [],
                styles: [],
            };
        ...
        checkPermissions() {
            ...
            this.has_invoice_permission = this.$root.getPermissionStatus(page_permissions?.permissions, "freezed_invoice_details")
        },

        cancelInvoice() {
            this.selected_subscription = this.subscription_details
            this.show_subscription_cancel_freeze_from = true
        }
    },
    watch: {
        freezedInvoiceDetails(data) {
            if (data != null) {
                console.log(data)
    ...
    mounted() {
        ...
        this.getFreezedInvoiceDetails(this.invoice_freezed_id)
    },
}
</script>